...
var getTooltipContent = function (state, chartData, activeIndex, activeLabel) {
    ...
    return graphicalItems.reduce(function (result, child) {
        ...
        return __spreadArray(__spreadArray([], result), [ChartUtils_1.getTooltipItem(child, payload)]);
    }, []);
};
        ...
        if (!result[axisId]) {
            ...
            if (dataKey) {
                ...
                if (type === 'category' && isCategorical) {
                    ...
                    var errorBarsDomain = ChartUtils_1.parseErrorBarsOfAxis(displayedData, graphicalItems.filter(function (item) { return item.props[axisIdKey] === axisId && !item.props.hide; }), dataKey, axisType, layout);
                    if (errorBarsDomain) {
                ...
                domain = ChartUtils_1.getDomainOfItemsWithSameAxis(displayedData, graphicalItems.filter(function (item) { return item.props[axisIdKey] === axisId && !item.props.hide; }), type, layout, true);
            }
            if (type === 'number') {
    ...
    var axisMap = graphicalItems.reduce(function (result, child) {
        ...
        if (!result[axisId]) {
            ...
            if (isCategorical) {
                ...
                domain = ChartUtils_1.parseSpecifiedDomain(Axis.defaultProps.domain, ChartUtils_1.getDomainOfItemsWithSameAxis(displayedData, graphicalItems.filter(function (item) { return item.props[axisIdKey] === axisId && !item.props.hide; }), 'number', layout), Axis.defaultProps.allowDataOverflow);
                domain = DetectReferenceElementsDomain_1.detectReferenceElementsDomain(children, domain, axisId, axisType);
            }
